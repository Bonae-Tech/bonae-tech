...
3. [Infraestructura en la nube](#3-infraestructura-en-la-nube)
   - [Caché de sitios Pages](#caché-de-sitios-pages)
...
- **Caché HTTP alineada al fingerprinting.** HTML con TTL corto; assets hasheados immutable; `sw.js` sin store. Ver [§ Caché de sitios Pages](#caché-de-sitios-pages) y la receta repetible [caching-pages.md](./caching-pages.md).
...
#### Caché de sitios Pages

Política compartida para apps Cloudflare Pages del monorepo (marketing Astro + admin Vite, y futuros sitios). **Receta paso a paso para aplicar en un sitio nuevo:** [caching-pages.md](./caching-pages.md).

Resumen:

| Ruta | `Cache-Control` | Motivo |
|------|-----------------|--------|
| `/*` (HTML / shell) | `public, max-age=60, must-revalidate` | Deploys visibles pronto |
| Assets hasheados (`/_astro/*` Astro, `/assets/*` Vite) | `public, max-age=31536000, immutable` | Fingerprint de contenido |
| `/sw.js` | `no-store` | El service worker debe actualizarse en cada deploy |
| `/manifest.webmanifest` (si existe) | `public, max-age=60, must-revalidate` | Alineado con HTML |

Service worker: navegaciones **network-first**; demás GET same-origin **cache-first**; no interceptar APIs (admin: `/content/*`). Nombre de caché `<prefix>-<shortSha>` vía `scripts/inject-sw-build-hash.mjs` después del build y antes de `wrangler pages deploy`.

| Sitio | `_headers` / `sw.js` | Prefijo SW |
|-------|----------------------|------------|
| Marketing (`bonae-tech`) | [`apps/static/public/`](../apps/static/public/) | `bonae-tech` |
| Admin (`bonae-admin`) | [`apps/admin/public/`](../apps/admin/public/) | `bonae-admin` |

**Panel Cloudflare (zona `bonaetech.com`):** [Caching → Configuration](https://dash.cloudflare.com/cd081958c621d4f8a9c7481da23e07f0/bonaetech.com/caching/configuration). Los headers por ruta en git (`_headers`) son la fuente de verdad; el dashboard complementa TTL de zona.
